...
- **边界**：不写具体用例（→ `test-case-writing`）；不代替用户裁决（自动化提案、预算裁决均 ⏸ 等确认）
...
- 按**维度**扫风险：功能域维度（权限 / 数据一致性 / 边界 / 状态流转 / 资损）+ 类型域轴名（并发 / 可靠 / 安全 / 性能 / 兼容 / 迁移 / 契约——与矩阵轴同源）
...
1. 写 `{项目}/测试策略.md`（Schema 结构 + Risk Map 内嵌；type_scope 每轴单行 flow 风格 + 足够性声明：逐轴档位动作清单与完成状态）
