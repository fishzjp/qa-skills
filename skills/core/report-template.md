...
> （同名区分：此处 S 系指 Bug 严重程度分级；类型决策矩阵的「S 级信号」〔semantic，语义信号复核〕是另一概念，见 `core/test-type-matrix.md`——两者同名不同义。）
...
> 类型域 handoff / 外部执行器 / blocked 轴的结果回收（决策见测试策略 type_scope，协议见 `core/test-type-matrix.md`）。无类型域专项时本节整体省略。
...
2. **执行类 Skill**（automated-e2e-testing / api-testing）：执行分报告至少包含 §2 执行统计 + §3 Bug 清单（Bug 条目字段与本模板一致），根因分析等五个扩展字段（§3 后五行）留 TODO 由 bug-analysis 补
3. **bug-analysis**：Bug 条目的"根因分析 / 影响范围 / Severity 依据 / 修复建议 / 回归建议"为本 Skill 的填写范围（条目字段结构以 bug-analysis 工作流的对齐版为准），结论按 `core/evidence.md` 标注 status
