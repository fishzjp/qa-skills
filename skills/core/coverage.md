...
> 类型性维度（11 / 13–16 / 18 / 19）的**纳排与深度决策已上收**至 `core/test-type-matrix.md`（test-strategy 产出 type_scope）——本文件的类型性维度仅作用例设计提示，不是范围决策来源。
...
### 2. 输入校验
必填缺失、格式非法、边界值（空/最大/最小/零值）、多值分隔符，是否都有对应的拦截用例？**每条校验规则是否在所有写入路径（创建/编辑/重新提交/导入）上都验证过，而不是只挂在创建？**（方法见 `core/methods/boundary.md`）
...
### 4. 状态流转
完整状态机的每一条边（A→B）是否覆盖？非法转换是否拒绝？并发状态变更是否有竞态条件？（方法见 `core/methods/state-machine.md`）
...
每条发现附 `文件:行` 证据并标注确信度，evidence 格式见 `core/evidence.md`：
...
以**零上下文新人**（没读过需求文档、没人讲解）为标准，逐项执行 `core/executability.md` 的定稿自检清单：文件头部有无导读四件套？每条用例**看得懂**（正文零代码内部、内部系统名与英文字段名可在术语表查到）？能否**开工执行**（页面入口写明、前置状态有制造方法、异步预期有判定时限、Mock 依赖写明找谁）？独立执行（有 UI）或协作执行（无 UI，标注请开发执行/反馈）？嵌入的是具体数据还是占位符？
...
### 9. 权限与角色（多角色系统）
普通用户、管理员、无权限用户的操作是否验证？角色变更后权限是否即时生效？（方法见 `core/methods/permission.md`）
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 1 性能效率 / 轴 4 并发一致性）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 2 业务安全；越权功能正确性归功能域 permission）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 5 兼容性）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 8 国际化）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 3 可靠性）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 10 契约与集成）。
...
> 类型纳排与深度决策以 `core/test-type-matrix.md` 为准（轴 9 迁移与升级）。
