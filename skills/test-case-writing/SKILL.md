...
提问格式与裁决落盘规则统一按 `../core/clarify-pattern.md`（此时加载）：逐条独立成卡、给出建议选项、用户答复记入澄清记录。
...
> 纪律依据：把「完整性」从阶段 4B 的后置审查纪律改为交付前的显式核对项——弱执行环境下后置纪律可能被跳过，逐模块核对拦住「写一个模块就收工」的早停。
...
- **覆盖维度**：**执行 `../core/coverage.md` 全部检查项**（核心 7 维 + 按项目类型的扩展维度 + 横切可执行性），不是可选参考
