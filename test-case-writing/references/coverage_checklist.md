...
全面代码审查发现的每个潜在 bug 是否都有验证用例？高风险点 D1-Dn 是否每条都有回归用例？死代码是否确认可忽略（不写用例）？文档偏离是否已确认（记入附录）？证伪项是否已核实（避免伪用例）？

**典型遗漏**：只测功能，未验证代码中的 null 未兜底 / 边界未防护（NaN/负数/空数组）/ 状态死锁等潜在 bug；或把已证伪的风险点（无下游消费）当成真 bug 写了用例。
