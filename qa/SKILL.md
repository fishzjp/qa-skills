---
name: qa
description: 用户说"帮我测试这个需求/功能"、"把这个功能完整测一遍"等端到端测试意图时的唯一入口：编排 需求理解 → 风险分析 → 测试策略 → 用例编写 → 用例审查 → 执行 → Bug 分析 → 回归验证 → 测试报告 的完整流水线，产出落盘、可断点续跑。只要某个单阶段产出（如"帮我审一下这份用例"）→ 直接用对应阶段 skill，不用本 skill。
---
...
- **反模式**：单会话内顺序累加加载全部 SKILL.md（8 个 × 数百行指令同时在场）
...
| 2 | 测试策略 | `test-strategy` | `{项目}/测试策略.md`（含 Risk Map） | — |
...
1. **不做裁决**：澄清、执行策略、Bug 定性一律呈现问题与建议后等用户，不代替用户决定
