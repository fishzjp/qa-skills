...
- **API 真实执行通过率 †**：如实披露的历史反向结果（74% vs 52%，3 采样口径；早期 87% 为 2 采样均值）。根因已定案并修复（2026-08-24 逐失败归类）：主因是评测任务契约的状态机断裂（创建恒返回待发布、领取要求已发布、契约却无发布路径——题目缺陷而非 skill 缺陷），叠加一次输出截断与一次采样退化；"严断言更易暴露失败"的早期解释经逐点证伪（46 个失败中仅 4 个与断言强度相关，且属超出书面契约的过严断言）。契约补全与截断续写修复后的弱模型段位复跑出现 31/38 个测试的满分采样，反转消除；主模型段位复验待评测通道恢复。
