...
description: 审查已有测试用例（存量资产、他人编写、AI 产出）的覆盖与质量时使用——独立评估功能性/边界/异常/权限/状态/回归/数据覆盖与可执行性，直接修订用例文件并留审查记录。不用于：从零写用例（test-case-writing）、写时自审（test-case-writing 阶段四）、端到端流水线（qa）。
---
...
- **直接在用例文件中修订**：补缺失用例（追加 TC 编号）、删除冗余、改正错误断言、补可执行性要素（导读区/时限/入口路径/具体数据）
- 修订后**重新抽取 Schema**（规则见 `../test-case-writing/SKILL.md` 的「Schema 抽取」）
- 文件末尾追加审查记录：

```markdown
## 审查记录（test-case-review {日期}）
- 基准：{黄金集标注 / 需求模型确认 / 自行提炼（未经确认）}
- 审查前：XX 条用例，XX 个模块
- 审查后：XX 条用例，XX 个模块
- 缺失（已补）：TC-xx…（{场景}）
- 冗余（已删）：TC-xx…
