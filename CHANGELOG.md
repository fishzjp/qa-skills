...
- **目录治理**：根目录缓存与系统垃圾清除（`.DS_Store`、`.pytest_cache`、`__pycache__`、空 `.benchmarks/`——均为运行时可再生文件）；2026-08-27 的两份 smoke 备份包（`.smoke_f2_backup.zip` 混合工程 fixture、`.smoke_f3_backup.zip` 畸形 schema fixture）自仓库根归档至 `eval/archive/`（本地评测区，不入库）；`.gitignore` 补 `.claude/`（此前仅靠贡献者全局 git ignore 兜底，换机裸克隆会露出未跟踪项，与 `.zcode/`/`.trae/` 同族）；`og.jpg` 归位 `assets/og.jpg`——落地页分享图与 hero / landing / 社交预览图统一收纳视觉资产目录，线上 URL 不变（`pages.yml` 打包时重命名回站点根 `og.jpg`，og:image 绝对 URL 与已流通分享链接均不受影响；触发路径、RELEASING 发版检查单、双语 README 结构图同步）。
...
- **全项目审查修复批（2026-09-07，六维审查：skill 内容 / 脚本与 CI / 门面一致性 / 过工程 / 安全 / 一致性横扫）**：
  ...
  - **新增架构红线 11**：`skills/` 下每个一级子目录必须含 SKILL.md（无 SKILL.md 的新目录此前逃过全部 per-skill 红线）。
  - **安装器自毁守卫物理路径化**：install.sh / uninstall.sh 的防自毁判定 `pwd` → `pwd -P`——`--target` 为链进仓库 skills/ 的 symlink 时旧守卫（逻辑路径）不命中，随后 `rm -rf` 可砸到源目录；安装器冒烟新增 symlink 攻击单元（install 与 uninstall 双向，锁死该修复）。
  - **安装单元清单单源化**：`SKILL_DIRS` 由 install.sh / uninstall.sh 双份手维护改为从 `skills/*/SKILL.md` 动态派生——消除"新增/下线 skill 必改两处"的失败模式（core 计数漏同步即此类模式的实例）。
  - **benchmark.yml 加固**（本地维护不入库、仅在含 eval/ 的私有环境运行）：workflow_dispatch inputs 与 secret 全部经 env 间接注入、不再直接插值进 `run:` 脚本（label/tasks 为任意字符串，直接拼 shell 是注入面）；补 `permissions: contents: read` 块。
  - **死资产清理**：删除 `assets/landing/` 13 张零引用配图（v2 落地页零本地图片引用，此前每次 pages 部署全量搬运、paths 触发器可被死图触发完整部署）；`.gitignore` 清除六行被 `eval/` 整目录规则完全覆盖的冗余行。
  - **计数与口径尾巴清扫**：`core/SKILL.md` 三处"其余 10 个 skill"→11（qa-memory 合入时"全计数面同步"漏掉 core 自身）；双语 README 结构树 methods/ 4→5 篇（data-factory 合入漏网）、scripts/ 行补 `validate_repo.py`（与 [Unreleased] 自登记载自相矛盾）、tests/ 行改为真实测试清单（原"validate_skills 的回归测试"名不副实）；落地页 / examples / AGENTS.md 的增益矩阵快照承诺对齐"按里程碑随 Release 附带"权威口径（v0.6.0 诚实化时漏改的残留）；RELEASING 反面实例更新（npm 0.6.0 已补发，现役断档为 v0.7.0 待 OTP）并同步触发路径；`validate_skills.py` 注释计数；README 贡献区自检行补双校验器；落地页"11 个领域 Skill"→"11 个 QA Skill"（qa 为薄编排、qa-memory 为治理单元，称"领域"不严谨）；package.json description 枚举补 pipeline orchestration。
  - **npm 发布机械门**：package.json 新增 `prepublishOnly`（publish 前强制通过两道校验器）——v0.5.1 / v0.6.0 / v0.7.0 三连"只发 GitHub、npm 断档"的流程性根治。
  - **CI 硬化小项**：label job 补 `timeout-minutes`；pip 依赖锁 `pyyaml>=6.0`。
...
- **第六次全面审查修复（口径对齐与接线补漏）**（2026-08-28）：README 双语能力清单「影响三面分析」订正为五面口径——bug-analysis 影响分析五面化早已完成，对外物料残留旧称造成对外承诺降级；落地页特性卡片两处口径订正为实测口径（「E2E 用例真实执行通过」「单任务×3 采样真浏览器实测」）；automated-e2e-testing SKILL.md 补 report-template「机读摘要」接线——执行分报告末节按占位符替换约定追加机读摘要片段，供 `qa` 收尾与后续回归编排机读解析（pipeline-integration「三文件互认」宣称的漏接点补齐）；triage.md 消费钩子表节名引用订正（「运行与验证节」→ api-testing 实际节名「§4 运行与结果」）；validate_schema.py docstring 与实际门行为对齐（--strategy 模式下 Critical/High 风险零覆盖 = 报错）。
...
- **全面审查修复**（第三轮审查闭环，2026-08-26）：**对外物料**——落地页 API 行改为复验口径
  ...
  类型域模块的标签注写法与矩阵轴 2 的标签规格。第四轮（漏网清单 / 新眼代理
  通读 / 假 HOME 沙箱 / 线上一致性与真实渲染）复审新增两处接线与措辞修复：
  exploratory-testing 异常定性接线 clarify-pattern（消费者名单早已宣称、与
  api-testing 同类漏接，场景词取枚举中的「Bug 定性」）；qa-skills-v2 注记
  「本节相关」修正为①②在本节、③在 §7.4 的显式归属；decision-layer-design
  行数预算句标注快照时点并补当前值（131/165 → 现 134/167）。渲染实测：线上页
  13 图全挂载、零 JS 错误、五个导航锚点齐全。
