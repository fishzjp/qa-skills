...
> **文档状态**：v1（2026-08-23）。文中 `eval/` 引用（运行归档、标注工具等）属**本地维护的评测链路**，不随公开仓库分发。**Phase A 已实施**（矩阵 `core/test-type-matrix.md`、扫描器 `core/scripts/scan_signals.py`、校验器 V1–V5、test-strategy 重写及 §6 全部同步改写，validate_skills 全绿，dogfood 链路验证通过）。**Phase B 前两轮完成**：GT 双人复核（GLM-5.2 独立标注，决策一致 40/45）→ 最弱模型首轮（Off 臂宽容口径亦为**零显式决策**——盲区在决策纪律不在类型知识；On 严格 0.533）→ **格式硬约束复验**（格式失败归零，查全 0.880，纯代码信号轴 8/9）；glm-5.2 梯度轮见 `eval/results/runs/`。开放问题状态见 §10。
>
> **与既有文档的关系**：
...
> **代码信号分两级**（已裁决，2026-08-23）：**G 级**（greppable）由 `core/scripts/scan_signals.py` 按每轴模式清单机械扫描，产出文件:行 级信号 + 默认档预填表——同代码必得同 G 级信号；**S 级**（semantic，信号清单中以〔S〕标注）是脚本抓不住的语义模式，由 agent 按固定清单复核。**R4 的"扫描全灭"必须 G + S 双确认**——S 级未复核不得作为 exclude 依据，否则脚本盲区会被制度化为漏测。这是"通过代码分析决定测什么"的落地形态，也是弱模型增益的第一机制（§5.4）。
>
...
1. **机械活全部脚本外包**（已裁决）：G 级信号查找、预填、校验全进脚本；S 级语义复核是 agent 的固定动作（每轴清单固定、照单执行）；弱模型只做"证据 → 决策"与"照单复核"两类受限判断——这是它够得着的认知负荷
...
1. **代码信号的落地形态**——**已裁决（2026-08-23）**。`core/scripts/scan_signals.py` 机械扫描（每轴 grep 模式清单 → 文件:行 信号 + 默认档预填表），agent 只做语义确认与补充；预填决策表与防橡皮图章约束（exclude 不可预填）见 §5.4。脚本随 Phase A 交付（§8）；边界：仅服务类型域信号扫描，不取代功能域的代码优先阅读。
...
### 10.6 战略储备：AI 应用特有失效模式轴（候选第 11 轴，未立项）

7. **AI 特有失效模式（幻觉 / prompt 注入 / 输出漂移 / 上下文污染）**——**backlog 待立项（2026-08-27）；所属环节：skills 主导，eval 随动；裁决时点：dogfooding 证据驱动**。被测对象含 LLM 组件（生成式输出、RAG、Agent 编排）的比例持续上升，而十轴矩阵对该类失效模式零覆盖；决策层机制本身为扩展设计（新增一轴 ≈ 六字段模板 + scan_signals 追加 grep 模式），边际成本远低于另造体系，因此只登记、不动工。**立项硬门槛**：AGENTS.md 每周 dogfooding 沉淀案例中 AI 失效域攒够 **≥3 例**方可启动——此前不做任何 skill 正文投入，防无题可做的过度设计。立项时六字段草案直接取用：

   - **需求信号**：PRD 含 LLM / 大模型 / 智能体 / RAG / 生成式输出质量条款等特征词；**代码信号**（并入 scan_signals grep 清单）：prompt 模板目录、LLM SDK 依赖（openai / langchain / anthropic 等）、向量库调用（pgvector / milvus / qdrant）；
   - **默认档**：软默认——G 信号命中预填 light，S 级人工确认后按风险定档；不设硬默认（AI 测试成本与误报率尚无业界共识基线）；
   - **档位语义**：light = 结构化属性断言替代相等断言（事实性 / 格式 / 安全边界逐项检查）；standard = 幻觉率阈值门限 + 注入攻击面用例（直接 / 间接 / 多轮三类）；full = RAG 双层评估（检索命中率 × 生成忠实度）+ 多次运行统计化判定（单次通过不构成证据）；
   - **执行归属**：agent 负责设计与裁决，执行交给项目自带评估脚本——不承诺统一执行器，规避 V5 空头支票；
   - **成本因子**：多次采样开销在十一轴中最高，full 档强制显式预算声明；
   - **阈值起点**（仅为开局参考，立项时以真实案例校准）：幻觉率按域别 医疗 <0.5% / 金融 <2% / 客服 <5%。

   纪律衔接：轴空间封闭原则不变，新增即矩阵版本升级（承接第 6 条），黄金集标注同步记录矩阵版本，GT 可比性不受破坏。
